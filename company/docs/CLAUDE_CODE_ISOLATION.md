...
Windows User/Machine CCR 관련 환경변수
기존 Enterprise 인증 상태
기존 Enterprise 모델 목록
...
5. CCR profile launcher로 기존 claude 실행
6. CCR 연결 실패 시 Enterprise로 자동 fallback하지 않고 명확히 실패
7. 세션 종료 후 service lifecycle 정책에 따라 유지 또는 stop
8. Enterprise baseline이 계속 정상인지 Doctor에서 확인
```
...
| `CLAUDE_APP_CONFIG_PERSISTENCE` | 실제 Claude-3p에 CCR 설정이 잔존 |
