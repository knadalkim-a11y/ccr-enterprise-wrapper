...
## TRAP-001 — Native Termux 결과를 Linux build 증거로 취급하지 말 것
...
- Root cause: native Termux의 Node는 `process.platform=android`이며 일반 Linux native addon 배포 대상과 다름
- Avoid: 외부 build 증거는 우선 Termux + PRoot Linux + Node 22에서 수집하고, 필요할 때만 별도 PC/CI를 사용함
...
Git·문서·에이전트 제어는 native Termux에서 계속 수행하고, native addon 증거만 적합한 runner로 분리한다.
